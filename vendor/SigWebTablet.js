...
function onDoneSign(customCallback) {
...
// function SigImageCallback( str ) { document.getElementById('sigImageData').value = str; }
window.onunload = window.onbeforeunload = (function(){ closingSigWeb(); });
function closingSigWeb() { ClearTablet(); SetTabletState(0, tmr); }
...
function isIE() {
    return (
        ...
        ((navigator.appName == 'Netscape') && (new RegExp("Trident/.*rv:([0-9]{1,}[\.0-9]{0,})").exec
...
function SigWebcreateXHR() {
    if (navigator.userAgent.indexOf ('Linux')!== -1) { console.log('Sigwebtablet not supported on Linux'); return; }
    try {
